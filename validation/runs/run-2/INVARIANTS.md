...
- Metrics, recommendation, and pipeline agree because they project one current collection.
- At most one recommendation is shown; it always refers to an existing record with a non-empty action.
- The recommendation explanation is derived from the same stage/deadline inputs as its score.
