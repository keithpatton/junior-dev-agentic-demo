...
const application={id:'proof',company:'Good Sort',role:'Junior Product Engineer',status:'Screen',deadline:'2026-08-17',nextAction:'Try the product',notes:'DO-NOT-PERSIST'};

for(const [boundary,project] of [['persistence',durable],['download',downloadable]]){
...
test('safe fields remain useful after projection',()=>{
  for(const project of [durable,downloadable]){
    const result=project([application])[0];
    assert.deepEqual([result.company,result.status,result.nextAction],['Good Sort','Screen','Try the product']);
  }
});
